...
let log, shorts_in, shorts;

let schedule_from, schedule_to, schedule_reset;
...
class Alarm extends utils.Adapter {
    ...
    async main() {
        ...
        if (this.config.circuits) {
            this.split_states(this.config.circuits);
        } else {
    ...
    enableSystem(_id, _state) {
        ...
        if (!this.config.opt_warning && is_alarm) {
            ...
            if (log) {
                this.log.info(`${this.config.log_act_not} ${names_alarm}`);
            }
            if (this.config.send_activation) {
                this.messages(`${this.config.log_act_not} ${names_alarm}`);
            }
        ...
        if (is_alarm) {
            ...
            if (log) {
                this.log.info(`${this.config.log_act_warn} ${names_alarm}`);
            }
            if (this.config.send_activated_with_warnings) {
                this.messages(`${this.config.log_act_warn} ${names_alarm}`);
            }
        } else {
    ...
    change(id, state) {
        ...
        if (notification_ids.includes(id) && this.isTrue(id, state, 'main')) {
            ...
            if (!activated && !inside && !night_rest) {
                return;
            }
            const name = this.get_name(id);
        ...
        if (send_instances.length) {
            ...
            send_instances.forEach(ele => {
                if (reg.test(ele) && this.config.opt_telegram) {
                    this.log.debug(
                        `Send message to ${ele} with special parameter, message: text: ${content}, user: ${this.user}, chatID: ${this.chatID}`,
                    );
    ...
    sayit(message, opt_val) {
        ...
        if (night_rest && this.config.opt_night_silent) {
            return;
        }
        if (tts_instance) {
        ...
        if (!inside && !burgle) {
            ...
            if (is_inside) {
                ...
                if (this.config.send_activation_warnings_inside) {
                    this.messages(`${this.config.log_warn_b_w} ${names_inside}`);
                }
                this.setState('info.log', `${this.config.log_warn_b_w} ${names_inside}`, true);
                if (log) {
                    this.log.info(`${this.config.log_warn_b_w} ${names_inside}`);
                }
                if (this.config.opt_say_names) {
    ...
    sleep_begin(auto) {
        ...
        if (is_notification) {
            ...
            if (this.config.send_activation_warnings_night) {
                this.messages(`${this.config.log_nights_b_w} ${names_notification}`);
            }
            this.setState('info.log', `${this.config.log_nights_b_w} ${names_notification}`, true);
            if (log) {
                this.log.info(`${this.config.log_nights_b_w} ${names_notification}`);
            }
            if (this.config.opt_say_names) {
    ...
    refreshLists() {
        ...
        this.check(zone_one_ids, 'zone_one', (_val, ids) => {
            ...
            if (ids.length > 0) {
                ...
                this.log.debug(`Names in zone one: ${names_zone_one}`);
        ...
        this.check(zone_two_ids, 'zone_two', (_val, ids) => {
            ...
            if (ids.length > 0) {
                ...
                this.log.debug(`Names in zone two: ${names_zone_two}`);
        ...
        this.check(zone_three_ids, 'zone_three', (_val, ids) => {
            ...
            if (ids.length > 0) {
                ...
                this.log.debug(`Names in zone three: ${names_zone_three}`);
        ...
        if (is_alarm) {
            this.setState('status.enableable', false, true);
        }
        if (this.config.opt_warning && is_alarm) {
            this.setState('status.enableable', true, true);
        }
        if (!is_alarm) {
            this.setState('status.enableable', true, true);
        }
    ...
    split_states(arr) {
        arr.forEach(ele => {
            if (ele.enabled) {
                if (ele.alarm) {
                    alarm_ids.push(ele.name_id);
                }
                if (ele.warning) {
                    inside_ids.push(ele.name_id);
                }
                if (ele.night) {
                    notification_ids.push(ele.name_id);
                }
                if (ele.leave) {
                    leave_ids.push(ele.name_id);
                }
            } else {
    ...
    check(arr, table, callback) {
        ...
        if (table === 'main') {
            ...
            tempStates = one_states;
        } else if (table === 'two') {
            tempStates = two_states;
        } else if (table === 'zone_one') {
            tempStates = zone_one_states;
        } else if (table === 'zone_two') {
            tempStates = zone_two_states;
        } else if (table === 'zone_three') {
            tempStates = zone_three_states;
        } else {
            this.log.warn(`Issue in function check, plaese report this the developer!`);
            return;
        }
    ...
    async get_zone_states() {
        ...
        if (this.config.zone_three) {
            this.config.zone_three.forEach(ele => {
                if (ele.enabled) {
                    zone_three_ids.push(ele.name_id);
                }
            });
        ...
        this.log.debug(
            `zone one: ${JSON.stringify(zone_one_states)} zone two: ${JSON.stringify(zone_two_states)} zone three: ${JSON.stringify(zone_three_states)}`,
        );
    }
        ...
        for (const item in presenceTimers) {
            ...
            if (presenceTimers.hasOwnProperty(item)) {
                switch (presenceTimers[item].option_presence) {
                    case 'time':
                        if (
                            presenceTimers[item].presence_time_from == '' ||
                            presenceTimers[item].presence_time_to == ''
                        ) {
                            this.log.warn(
                                `Please check the times when configuring attendance: ${presenceTimers[item].name} -- ${presenceTimers[item].name_id} `,
                            );
                            return;
                        }
                        if (
                            ...
                            !presenceTimers[item].wasOn
                        ) {
                            this.log.debug(
                                `Delay for: ${presenceTimers[item].name} -- ${presenceTimers[item].name_id}  starts ${presenceTimers[item].presence_delay}ms, because time is in range.`,
                            );
                            presenceTimers[item].wasOn = true;
                            presenceTimers[item].presenceDelayTimer = setTimeout(() => {
                                this.log.debug(
                                    `Delay for: ${presenceTimers[item].name} -- ${presenceTimers[item].name_id}  ends and switch ON ${presenceTimers[item].presence_length}ms.`,
                                );
                                this.setForeignState(
                                    presenceTimers[item].name_id,
                                    this.bools(presenceTimers[item].presence_val_on),
                                    err => {
                                        if (err) {
                                            this.log.warn(`Cannot set state: ${err}`);
                                        }
                                ...
                                presenceTimers[item].presenceLengthTimer = setTimeout(() => {
                                    this.log.debug(
                                        `Switch ON for: ${presenceTimers[item].name} -- ${presenceTimers[item].name_id}  ends and switch OFF.`,
                                    );
                                    this.setForeignState(
                                        presenceTimers[item].name_id,
                                        this.bools(presenceTimers[item].presence_val_off),
                                        err => {
                                            if (err) {
                                                this.log.warn(`Cannot set state: ${err}`);
                                            }
                            ...
                            this.log.debug(
                                `${presenceTimers[item].name} -- ${presenceTimers[item].name_id}  was ON or is not in time range`,
                            );
                        }
                        break;
                    case 'sunrise':
                        if (sunrise && !presenceTimers[item].wasOn) {
                            this.log.debug(
                                `Delay for: ${presenceTimers[item].name} -- ${presenceTimers[item].name_id}  starts ${presenceTimers[item].presence_delay}ms, by sunrise`,
                            );
                            presenceTimers[item].wasOn = true;
                            presenceTimers[item].presenceDelayTimer = setTimeout(() => {
                                this.log.debug(
                            ...
                            this.log.debug(
                                `${presenceTimers[item].name} -- ${presenceTimers[item].name_id}  was ON or is no sunrise`,
                            );
                        }
                        break;
                    case 'sunset':
                        if (sunset && !presenceTimers[item].wasOn) {
                            this.log.debug(
                                `Delay for: ${presenceTimers[item].name} -- ${presenceTimers[item].name_id}  starts ${presenceTimers[item].presence_delay}ms, by sunset`,
                            );
                            presenceTimers[item].wasOn = true;
                            presenceTimers[item].presenceDelayTimer = setTimeout(() => {
                                this.log.debug(
                            ...
                            this.log.debug(
                                `${presenceTimers[item].name} -- ${presenceTimers[item].name_id}  was ON or is no sunset`,
                            );
                        }
                        break;
                    case 'light':
                        // eslint-disable-next-line no-case-declarations
                        const lightVal = await this.getForeignStateAsync(
                            presenceTimers[item].presence_trigger_light,
                        ).catch(e => {
                            this.log.warn(
                                `Check your light ID ${presenceTimers[item].name} -- ${presenceTimers[item].name_id}  in presence config! +++ ${e}`,
                            );
                            return;
                        });
                        if (lightVal.val < presenceTimers[item].presence_light_lux && !presenceTimers[item].wasOn) {
                            this.log.debug(
                                `Delay for: ${presenceTimers[item].name} -- ${presenceTimers[item].name_id}  starts ${presenceTimers[item].presence_delay}ms, because light value is not under the limit.`,
                            );
                            presenceTimers[item].wasOn = true;
                            presenceTimers[item].presenceDelayTimer = setTimeout(() => {
                                this.log.debug(
                            ...
                            this.log.debug(
                                `${presenceTimers[item].name} -- ${presenceTimers[item].name_id}  was ON or light value is not under the limit.`,
                            );
                        }
                        break;
                    default:
                        this.log.warn(
                            `Please check presence configuration for: ${presenceTimers[item].name} -- ${presenceTimers[item].name_id} , value: ${presenceTimers[item].option_presence}`,
                        );
                }
            }
        }
